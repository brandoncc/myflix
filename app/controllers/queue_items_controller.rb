class QueueItemsController < ApplicationController
  ...
  def create
    ...
    if video
      QueueItem.create(user: current_user,
                       video: video,
                       position: QueueItem.next_available_position(current_user))
      flash[:success] = "#{video.title} was successfully added to your queue."
    else
      flash[:danger] =
        'There was a problem adding the video to your queue. Please try again.'
    end
  ...
  def destroy
    if current_user == QueueItem.find(params[:id]).user
      ...
      flash[:danger] =
        'There was a problem removing the video from your queue.'
    end
    redirect_to :back
  end
end
